...
export async function addNote(
  ...
  try {
    ...
    const query = `
      INSERT INTO note (notes, merchant_id, outlet_id)
      VALUES ($1, $2, $3) RETURNING *;
    `;
    const values = [notes, merchant_id, outlet_id || null];
...
export async function updateNote(
  ...
  try {
    ...
    const query = `
      ...
      RETURNING *;
    `;
    const values = [notes, id, merchant_id, outlet_id || null];
    const result = await client.query(query, values);
    return result.rows[0];
  } finally {
    client.release();
